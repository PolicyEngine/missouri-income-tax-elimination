 ...
 *
 * For the Missouri Income Tax Elimination dashboard, the reform is supplied
 * directly by the user through the reform builder. The household impact is
 * calculated as (reform) minus (baseline current law), so positive numbers
 * mean the household nets more under the reform.
 */
...
/** Default reform is empty (current law baseline with no changes). */
...
export function buildHouseholdSituation(
  ...
  const effectiveStateCode = state_code || 'MO';
...
/** Default reform policy (empty — user supplies one via the reform builder). */
